<template>
  <b-card header="EDIT CATEGORY">
    <b-form @submit.prevent="submit(form)">
      ...
      <form-button type="submit" variant="primary" :loading="loading">Edit category</form-button>
...
export default {
  ...
  async asyncData({ $axios, params }) {
    const id = params.id
    const category = await $axios.$get('admin/categories/' + id)
    return {
      id: category.id,
      form: {
        name: category.name,
  ...
  methods: {
    async submit(data) {
      ...
      try {
        await this.$axios.$put('/admin/categories/' + this.id, data)
